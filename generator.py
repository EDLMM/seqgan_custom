...
class Generator(object):
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim,
                 ...
                 learning_rate=0.01, reward_gamma=0.95,randomEmb=True):
        ...
        with tf.variable_scope('generator'):
            # just use randomized vector to represent words
            # possible to implement other word2vec transformation
            # num_emb == vocab_size, emb_dim is hyper parameter
            if randomEmb:
                self.g_embeddings = tf.Variable(self.init_matrix([self.num_emb, self.emb_dim]))
            else:
                self.embedding_placeholder= tf.placeholder(tf.float32, [self.num_emb, self.emb_dim])
                self.g_embeddings = tf.Variable(tf.constant(0.0, shape=[self.num_emb, self.emb_dim]), trainable=True, name="g_embeddings")
        ...
        def _g_recurrence(i, x_t, h_tm1, gen_o, gen_x):
            ...
            log_prob = tf.log(tf.nn.softmax(o_t)) # regularize the scores to probabilities at [0.00,1.00]
            # log_prob.size = [batch_size,num_classes]
            # next_token.size = [batch_size, intented_num] (here is intended_num=1)
        ...
        # tf.gradients(ys,xs): return dy/dx
